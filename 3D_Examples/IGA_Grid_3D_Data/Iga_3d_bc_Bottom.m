function [A,rhs]=Iga_3d_bc_Bottom(A,rhs,N1,N2,N3,n_dofs)
...
for k=1:N3
 ...
 be= n_dofs + [bottom_face_dof,front_face_dof,back_face_dof,left_face_dof,right_face_dof];

be=unique(be);
A(be,:)=zeros;A(:,be)=zeros;rhs(be)=zeros;
A(be,be)=speye(length(be),length(be));
