function [P,S]= IGA_DG_Top_Bottom_Face_Assemble(nurbs_original_1,nurbs_original_2,nurbs_refine_1,nurbs_refine_2, n_dofs)
%%
...
for i = 1:NoEs_u
 ...
 ele_idx_1     = u_edge_element_idx_1(i) + ( v_edge_element_idx_1(j)  -1 )*uNoEs_1; % The element index in the coarser mesh from patch 1
 ele_idx_2     = u_edge_element_idx_2(i) + ( v_edge_element_idx_2(j)  -1 )*uNoEs_2; % The element index in the coarser mesh from patch 2
 ele_idx_on_edge_1( merge_ele_idx ) = ele_idx_1 ;
 ele_idx_on_edge_2( merge_ele_idx ) = ele_idx_2 ;
 merge_coordinate(merge_ele_idx,:)  = [Ubreaks(i:i+1), Vbreaks(j:j+1)];
end
end
...
top_face_eles_2layers_dofs_plus       = nurbs_refine_1.top_face_eles_2layers_dofs;
bottom_face_eles_2layers_dofs_minus   = nurbs_refine_2.bottom_face_eles_2layers_dofs;
...
%%
w_top = 1.0;
Wders_plus  = bspbasisDers(knotW_1,pw_1,w_top,1); % For lower volume, the top face:    w = 1
Nw_plus     = Wders_plus(1,end-1:end);  %%  This needs special attention
DNw_plus    = Wders_plus(2,end-1:end);
%%
w_bottom = 0.0;
Wders_minus = bspbasisDers(knotW_2,pw_2,w_bottom,1); % For upper volume, the bottom face: w = 0
Nw_minus    = Wders_minus(1,1:2);
DNw_minus   = Wders_minus(2,1:2);
...
normal = [0;0;1];  % The unit outward normal is upwards

for e = 1:NoEs
    ue = merge_coordinate(e,1:2);  ve = merge_coordinate(e,3:4);
...
%     disp(ve)
    ...
    face_dofs_plus  = top_face_eles_2layers_dofs_plus(face_idx_plus,:);
    face_dofs_minus = bottom_face_eles_2layers_dofs_minus(face_idx_minus,:);
...
[~,DF_plus]  = NurbsVolume(ConPts_o_1,weights_o_1,knotU_o_1,pu_o_1,u,knotV_o_1,pv_o_1,v,knotW_o_1,pw_o_1,w_top);
[~,DF_minus] = NurbsVolume(ConPts_o_2,weights_o_2,knotU_o_2,pu_o_2,u,knotV_o_2,pv_o_2,v,knotW_o_2,pw_o_2,w_bottom);
...
J_plus = sqrt(A*A+B*B+C*C)*uJ*vJ*gw(i)*gw(j);
  ...
  Dv_w_basis_plus  = DNv_plus'* Nw_plus; 
  
  Dv_w_basis_plus = Dv_w_basis_plus(:)'; 
