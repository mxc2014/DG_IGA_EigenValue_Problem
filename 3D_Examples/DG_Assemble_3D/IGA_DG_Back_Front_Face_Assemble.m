function [P,S]= IGA_DG_Back_Front_Face_Assemble(nurbs_original_1,nurbs_original_2,nurbs_refine_1,nurbs_refine_2, n_dofs)
%%
...
Vbreaks      = [VBreaks_1,VBreaks_2];
Vbreaks      = unique(Vbreaks);
NoEs_v       = length(Vbreaks)-1;
v_edge_element_idx_1 =  Merge_edge_elements_idx(VBreaks_1,Vbreaks);
v_edge_element_idx_2 =  Merge_edge_elements_idx(VBreaks_2,Vbreaks);
...
for j = 1:NoEs_w
for i = 1:NoEs_v
 merge_ele_idx = i + (j-1)*NoEs_v;  % The element index in the finner mesh
 ele_idx_1     = v_edge_element_idx_1(i) + ( w_edge_element_idx_1(j)  -1 )*vNoEs_1; % The element index in the coarser mesh from patch 1
 ele_idx_2     = v_edge_element_idx_2(i) + ( w_edge_element_idx_2(j)  -1 )*vNoEs_2; % The element index in the coarser mesh from patch 2
 ele_idx_on_edge_1( merge_ele_idx ) = ele_idx_1 ;
 ele_idx_on_edge_2( merge_ele_idx ) = ele_idx_2 ;
 merge_coordinate(merge_ele_idx,:)  = [Vbreaks(i:i+1), Wbreaks(j:j+1)];
end
end
...
front_face_eles_2layers_dofs_plus        = nurbs_refine_1.front_face_eles_2layers_dofs;
back_face_eles_2layers_dofs_minus      = nurbs_refine_2.back_face_eles_2layers_dofs;

%  nurbs_refine_1.Element
%  disp('=====================')
%  back_face_eles_2layers_dofs_plus
%  disp('=====================')
%  front_face_eles_2layers_dofs_minus


np      = max(pu_1,pu_2) +1 ;
[gp,gw] = grule(np);

%%  The front face of the back volume
u_back = 1.0;
Uders_plus  = bspbasisDers(knotU_1,pu_1,u_back,1); % For lower volume, the top face:    w = 1
Nu_plus     = Uders_plus(1,end-1:end);  %%  This needs special attention
DNu_plus    = Uders_plus(2,end-1:end);
%%
u_front     = 0.0;
Uders_minus = bspbasisDers(knotU_2,pu_2,u_front,1); % For upper volume, the bottom face: w = 0
Nu_minus    = Uders_minus(1,1:2);
DNu_minus   = Uders_minus(2,1:2);
%%
...
normal = [1;0;0];  % The unit outward normal is rightward

for e = 1:NoEs
    ve = merge_coordinate(e,1:2);  we = merge_coordinate(e,3:4);
...
%     disp(ve)
    ...
    face_dofs_plus  = front_face_eles_2layers_dofs_plus(face_idx_plus,:);
    face_dofs_minus = back_face_eles_2layers_dofs_minus(face_idx_minus,:);
...
[~,DF_plus]  = NurbsVolume(ConPts_o_1,weights_o_1,knotU_o_1,pu_o_1,u_back,knotV_o_1,pv_o_1,v,knotW_o_1,pw_o_1,w);
[~,DF_minus] = NurbsVolume(ConPts_o_2,weights_o_2,knotU_o_2,pu_o_2,u_front,knotV_o_2,pv_o_2,v,knotW_o_2,pw_o_2,w);
...
J_plus = sqrt(A*A+B*B+C*C)*vJ*wJ*gw(i)*gw(j);
