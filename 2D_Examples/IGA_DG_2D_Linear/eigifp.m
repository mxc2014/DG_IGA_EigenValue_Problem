...
%
%
%   EIGIFP (version 2.2):  computes a few (algebraically) smallest or largest 
...
%
%   A (and/or B) can be a sparse matrix, or a function/function handle  
%   that returns A*x for x. In the latter case, the dimension of A has 
...
%       user supplied preconditioner. It can be a matrix or a function/function handle. 
%       If it is a matrix, it should be L in an approximate LDL^T 
%       factorization of A-mu*B with D being a diagonal of plus or minus 1.
%       If it is a function, it should be approximately L\(L^T\x). 
%     opt.DISP:
%       Set to 0 to disable on-screen display of output, and to any other 
%       numerical value to enable display. Default: 0
%     opt.MAXEIG:
...
%   Version 2.2 is dated June 2018
...
elseif (isa(A, 'function_handle') || ischar(A))
    usePrecon = 0;
else
...
if ( nargin>1 )
    if ( (~ isstruct(varargin{2})) ) 
        if( isstr(varargin{2}) || (any(size(varargin{2}) ~= 1)) || isa(varargin{2}, 'function_handle'))
        B=varargin{2};
        if ( isnumeric(B) )
            if ( isempty(n) )
                n=size(B,1);
        ...
        elseif (isa(B, 'function_handle') || ischar(B))
            usePrecon = 0;
        else
...
if ( isempty(normB) )
    ...
    elseif (isa(B, 'function_handle'))
        normB=norm(B(ones(n,1)), 1)/n; 
        fprintf(1,'Warning: options.Bnorm (estimate of ||B||_1) is not provided.\n');   
    elseif ( isstr(B) )
...
% toleranceA=10*eps*normA*sqrt(n); % The inital tolerance
% toleranceA=(1e-6)*eps*normA*sqrt(n);

toleranceA = 1.0e-14;

if(~isempty(B)) 
%    toleranceB=10*eps*normB*sqrt(n); % The inital default
%      toleranceB=(1e-2)*eps*normB*sqrt(n);
...
if (isnumeric(A))       %if given a matrix, use (A,B) or (-A,B) if maxeig =1
    ...
    if (findMax==1)     % here we use minusA and negate the results when done
        ...
        [EVals, EVecs] = ifree(nA,B,n,m,toleranceVec,iterMax,k,X,eta,L,usePrecon,normA,normB,outputYes);
        EVals=-EVals;
        if (outputYes)
            fprintf(1,'  Negating the eigenvalues of (-A, B) => the largest eigenvalues of (A,B):\n');
            fprintf(1,'  %e\n',EVals);
